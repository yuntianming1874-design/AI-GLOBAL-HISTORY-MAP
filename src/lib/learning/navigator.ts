...
function causeEventCandidates(e: { civilizationId: string; year: number }, limit: number) {
  // earlier same-civ events (preconditions), most recent first
  return events
    .filter((x) => x.civilizationId === e.civilizationId && x.year < e.year)
...
export function buildRecommendations(
  ...
  if (context.eventId) {
    ...
    if (e) {
      ...
      // cause: earlier same-civ event
      const cause = causeEventCandidates(e, 1)[0];
      if (cause) {
        push(
          make(
            "cause",
  ...
  if (context.year !== null && out.length < limit) {
    const yearEvents = events
      .filter((e) => e.year === context.year && e.significance >= 4)
